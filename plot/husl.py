...
# Raw code taken from the python port of the HUSL colorspace at
# https://github.com/boronine/pyhusl. I've adapted it to work for Python3.
...
def husl_to_rgb(h, s, l): return lch_to_rgb(*husl_to_lch([h, s, l]))
def husl_to_hex(h, s, l): return rgb_to_hex(husl_to_rgb(h, s, l))

def rgb_to_husl(r, g, b): return lch_to_husl(rgb_to_lch(r, g, b))
def hex_to_husl(hex): return rgb_to_husl(*hex_to_rgb(hex))

def huslp_to_rgb(h, s, l): return lch_to_rgb(*huslp_to_lch([h, s, l]))
def huslp_to_hex(h, s, l): return rgb_to_hex(huslp_to_rgb(h, s, l))

def rgb_to_huslp(r, g, b): return lch_to_huslp(rgb_to_lch(r, g, b))
def hex_to_huslp(hex): return rgb_to_huslp(*hex_to_rgb(hex))



def lch_to_rgb(l, c, h): return xyz_to_rgb(luv_to_xyz(lch_to_luv([l, c, h])))
...
def xyz_to_rgb(triple):
    xyz = [x for x in map(lambda row: dot_product(row, triple), m)]
    return [x for x in map(from_linear, xyz)]

def rgb_to_xyz(triple):
    rgbl = [x for x in map(to_linear, triple)]
    return [x for x in map(lambda row: dot_product(row, rgbl), m_inv)]

def xyz_to_luv(triple):
    X, Y, Z = triple
    
    if X == 0.0 and Y == 0.0 and Z == 0.0: return [0.0, 0.0, 0.0]
...
def lch_to_husl(triple):
    ...
    if L > 99.9999999: return [H, 0.0, 100.0]
    if L < 0.00000001: return [H, 0.0, 0.0]
