...
Data Mining, Inference, and Prediction, Second Edition: Chapter 6.
...
def _lowess_initial_fit(x_copy, y_copy, k, n):
   ...
   """
    ...
    for i in range(n):
        ...
        X = x_copy[nn_indices[0]:nn_indices[1]]
        y_i = weights[i, :] * y_copy[nn_indices[0]:nn_indices[1]]

        beta = np.polyfit(weights[i, :] * X, y_i, 2)
        fitted[i] = np.polyval(beta, x_copy[i])
...
def _lowess_robustify_fit(x_copy, y_copy, fitted, weights, k, n):
   ...
   Returns
    ...
    for i in range(n):
        ...
        beta = np.polyfit(total_weights * X, y_i, 2)
